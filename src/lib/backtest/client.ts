...
import { BacktestRequest, BacktestResponse, BacktestStatus, BacktestResult } from './types'
...
export class BacktestServiceClient {
  ...
  async submitBacktest(request: BacktestRequest): Promise<BacktestResponse> {
    try {
      const response = await this.makeRequest('/api/backtest', {
        method: 'POST',
        body: JSON.stringify(request),
      })

      if (!response.ok) {
        throw new BacktestError(`HTTP ${response.status}: ${response.statusText}`)
      }

      return await response.json()
    } catch (error) {
      throw new BacktestError(`Failed to submit backtest: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getBacktestStatus(backtestId: string): Promise<BacktestStatus> {
    try {
      ...
      throw new BacktestError(`Failed to get backtest status: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getBacktestResults(backtestId: string): Promise<BacktestResult> {
    try {
      const response = await this.makeRequest(`/api/backtest/${backtestId}/results`)

      if (!response.ok) {
        if (response.status === 404) {
          throw new BacktestError(`Backtest results for ${backtestId} not found`)
        }
        throw new BacktestError(`HTTP ${response.status}: ${response.statusText}`)
      }
      ...
      throw new BacktestError(`Failed to get backtest results: ${this.getErrorMessage(error)}`)
    }
  }
   ...
   */
  async cancelBacktest(backtestId: string): Promise<{ success: boolean; message: string }> {
    try {
      const response = await this.makeRequest(`/api/backtest/${backtestId}`, {
        method: 'DELETE',
      })
      ...
      throw new BacktestError(`Failed to cancel backtest: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getQueueStatus(): Promise<{
    ...
    try {
      ...
      throw new BacktestError(`Failed to get queue status: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getAvailableStrategies(): Promise<Array<{
    ...
    try {
      ...
      throw new BacktestError(`Failed to get available strategies: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getBacktestHistory(params: {
    ...
    try {
      ...
      if (params.strategyName) queryParams.append('strategy', params.strategyName)

      const url = `/api/backtest/history${queryParams.toString() ? '?' + queryParams.toString() : ''}`
      ...
      throw new BacktestError(`Failed to get backtest history: ${this.getErrorMessage(error)}`)
    }
  }
  ...
  async getServiceMetrics(): Promise<{
    ...
    try {
      ...
      throw new BacktestError(`Failed to get service metrics: ${this.getErrorMessage(error)}`)
    }
  }
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (status.status === 'failed' || status.status === 'cancelled') {
          throw new BacktestError(`Backtest ${status.status}: ${status.error || 'Unknown error'}`)
        }
        ...
        throw new BacktestError(`Polling failed: ${this.getErrorMessage(error)}`)
      }
    }

    throw new BacktestError('Backtest polling timeout - operation may still be running')
  }
